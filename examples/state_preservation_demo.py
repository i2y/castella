"""Demo: State Preservation with ListState.map_cached()

This demo shows how to preserve widget state using the ListState.map_cached() method.
map_cached() automatically reuses widget instances across view() rebuilds.

Pattern:
- Use self._items.map_cached(factory) in view()
...
class StatefulDemo(Component):
    """Demo using ListState.map_cached() for state preservation."""
    ...
    def view(self) -> Widget:
        header = Column(
            Text("State Preservation Demo").fixed_height(40),
            Row(
        ...
        # Use map_cached() to reuse widget instances across view() rebuilds
...
def main():
    print("=" * 60)
    print("State Preservation Demo (ListState.map_cached())")
    print("=" * 60)
    print()
    print("This demo uses ListState.map_cached() to preserve widget state")
