"""Diff viewer widget for wrks."""

import re
from dataclasses import dataclass
from typing import Optional

from castella import Column, Row, Text, Widget, Spacer, SimpleText
...
class DiffView(Component):
    """Displays diff output with syntax highlighting like Claude Code."""
    ...
    def view(self) -> Widget:
            ...
            widgets.append(
                Column(
                    Row(
                        ...
                        Text(header_text, font_size=12)
                        .text_color(theme.colors.text_primary)
                        .fixed_height(20)
                        .height_policy(SizePolicy.FIXED),
                    ).fixed_height(20).height_policy(SizePolicy.FIXED),
                    Row(
                        Spacer().fixed_width(16),
                        Text(f"└ {stats_text}", font_size=11)
                        .text_color(theme.colors.border_secondary)
                        .fixed_height(18)
                        .height_policy(SizePolicy.FIXED),
                    ).fixed_height(18).height_policy(SizePolicy.FIXED),
        ...
        if line_count >= self._max_lines:
            widgets.append(
                Text(f"... (truncated, {self._max_lines}+ lines)", font_size=10)
                .text_color(theme.colors.border_secondary)
                .fixed_height(18)
                .height_policy(SizePolicy.FIXED)
            )
    ...
    def _build_raw_diff(self, theme) -> Widget:
        ...
        if len(lines) > self._max_lines:
            ...
            lines.append(f"... ({len(self._diff_text.split(chr(10))) - self._max_lines} more lines)")
